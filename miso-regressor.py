...
def get_param(model, mode):
    if model == "KNN" and mode == 'Normal':
        ...
        param_grid = {'n_estimators' : list(range(50, 310, 10)), 'min_samples_split': list(range(1, 5, 1))}
        return param_grid
    elif model == 'GBR' and mode == 'Abnormal':
        param_grid = {'n_estimators' : list(range(50, 310, 10)), 'min_samples_split': list(range(1, 5, 1)),'learning_rate': list(np.arange(0.05, 0.21, 0.01))}
        return param_grid
    elif model == 'ABR' and mode == 'Abnormal':
        param_grid = {'n_estimators' : list(range(50, 310, 10)),'learning_rate': list(np.arange(0.05, 0.21, 0.01))}
        return param_grid
    else:
        return None
...
def root():
    ...
    if dataset != None and df1.empty == False and df1.shape[1]>1:
    ...
    if 1+1 == 2:
        ...
        if choice == '1. View Data':
            ...
            if dataset is not None and df.shape[1]>1 and df.shape[0]>15:
                ...
                objExist = df_types.isin(["object"]).any().any()
                ...
                if st.checkbox("Important Features") and df.shape[1]>1 and df.shape[0]>1:
                    X = df.iloc[:,0:-1]
                    Y = df.iloc[:,-1]
                    #Revisit
                    if Y.dtype == "float64" and objExist != True:
                ...
                if st.checkbox("Show Models"):
                    ...
                    if st.checkbox("Tune Model"):
                        ...
                        if Best_Model == "KNN":
                            ...
                            MSE = round(mean_squared_error(Y_validation, Y_prediction_ori),2)
