...
num_folds = 5
scoring = 'r2'
tmppath = "C:\\Users\\"+str(os.environ.get('USERNAME'))+"\\AppData\\Local\\Temp"
    ...
    if 1+1 == 2:
        ...
        if choice == '2. Build Model':
            if dataset is not None and df.shape[1]>1:
                ...
                #Model_Frame = Model_Frame.sort_values(by=['MSE'], ascending = True)
                ...
                if st.checkbox("Show Models"):
                    ...
                    if st.checkbox("Tune Best Model"):
                        ...
                        if Best_Model == "KNN":
                            ...
                            pickle.dump(grid, open(os.path.join(tmppath, filename), 'wb'))
        ...
        if choice == '3. Predict':
            if df is not None:
                if dataset is not None and df.shape[1]>1:
                    filename = 'finalized_model.sav'
                    grid = pickle.load(open(os.path.join(tmppath, filename), 'rb'))
                    ...
                    if st.button("Predict"):
                        ...
                        st.write("Model stored in " + str(os.path.join(tmppath, filename)))
